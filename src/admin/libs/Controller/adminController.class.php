<?php
	class adminController{
		...
		public function login(){/* 管理员登陆管理函数 */
		...
		public function newsadd(){/* 添加文章控制层函数 */
			if(!isset($_POST['submit'])){
				$data = $this->getnewsinfo();/* 获取文章信息 */
		...
		public function newslist(){/* 生成文章列表 */
			$data = $this->getnewslist();
			VIEW::assign(array('data'=>$data));/* 调用smarty函数 */
			VIEW::display('admin/newslist.html');
		}

		public function newsdel(){/* 删除文章控制层函数 */
			if($_GET['id']){
				$this->delnews();
		...
		private function checklogin(){/* 处理管理员登录信息 */
		...
		private function newssubmit(){/* 提交文章信息处理函数 */
			extract($_POST);				
